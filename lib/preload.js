...
const core = new _core.Core(); // All of the Node.js APIs are available in the preload process.
...
let slicedFields; // console文字の色指定

const consoleColorRed = '\u001b[31m';
const consoleColorReset = '\u001b[0m'; // 項目シートの列定義　※エクセルの列の増減をやりやすくするためのオブジェクト
...
const sheetCol = {
  ...
  valueSetName: "L",
  //グローバル選択リスト値
  valueSet_valueSetDefinition_value: "M",
  //選択リスト値
  valueSetDefault: "N",
  //選択リスト先頭行デフォルト設定
  valueSet_valueSetDefinition_sorted: "O",
  //選択リストアルファベット順ソート
  valueSet_restricted: "P",
  //値セットで定義された値に選択リストを制限します
  valueSet_controllingField: "Q",
  //制御項目
  valueSet_valueSettings: "R",
  //項目の連動関係
  formulaType: "S",
  //数式：戻り値
  formula: "T",
  //数式
  formulaTreatBlanksAs: "U",
  //数式：空白時
  displayFormat: "V",
  //自動採番：表示形式
  startingNumber: "W",
  ...
  summarizedField: "Z",
  //積み上げ集計：積み上げ項目
  summaryFilterItems: "AA",
  //積み上げ集計：検索条件
  referenceTo: "AB",
  //主従と参照関係
  relationshipLabel: "AC",
  //主従と参照関係：項目の表示ラベル
  relationshipName: "AD",
  //主従と参照関係：子リレーション名
  lookupFilter: "AE",
  //主従と参照関係：ルックアップ検索条件
  deleteConstraint: "AF",
  //参照関係：参照レコードが削除された場合の対処方法
  reparentableMasterDetail: "AG",
  //主従関係：親の変更を許可
  writeRequiresMasterRead: "AH",
  //主従関係：共有設定
  defaultValueCheckBox: "AI",
  //初期値：チェックボックス
  defaultValueFormula: "AJ",
  //初期値：数値・テキスト
  description: "AK",
  //説明
  profileName1: "AL5",
...
function JSforceUpsert() {
  // 初期化
  getFromValue();
  upsertResultText = ""; // 情報が足りないときは実行を中止する
  ...
  console.log(sheet); // カスタム項目用配列の宣言（中身はオブジェクトの配列）

  const customFields = getCustomFieldsFromXlsx();
  console.log(customFields); // フィールドレベルセキュリティ設定用プロファイル定義配列 Admin=システム管理者

  const profiles = getProfilesFromXslx(); //　エクセルからデータを登録する際にはこちらの行を有効にする

  console.log(profiles); // フィールドレベルセキュリティ用オブジェクト配列の宣言

  const fieldPermissions = getPermissionsFromXslx(profiles);
  console.log(fieldPermissions); // ログインURLを設定
...
async function upsert(conn, customFields, profiles, fieldPermissions) {
  for (let i = 0; i < customFields.length; i += 10) {
    slicedFields = customFields.slice(i, i + 10);
    await conn.metadata.upsert('CustomField', slicedFields).then(results => {
...
function security(conn, profiles, fieldPermissions) {
  for (let i = 0; i < profiles.length; i++) {
    ...
    conn.metadata.update('Profile', {
      fullName: profiles[i],
      fieldPermissions: fieldPermissions[i].profilePermisson
    }).then(results => {
...
function getCustomFieldsFromXlsx() {
  ...
  for (let i = 7; i <= excelCol; i++) {
    //i はエクセルの項目の開始行
    if (sheet[sheetCol.label + i] && !sheet[sheetCol.FIX + i]) {
      //A列にデータが存在しておりFIXではないか確認
      // 配列にオブジェクトを追加
      fields.push({}); // ラベル（label）

      fields[cnt].label = sheet[sheetCol.label + i]['v']; // 項目名（fullName）
      // エクセルA3からオブジェクト名を取得して連結する

      fields[cnt].fullName = sheet['A3']['v'] + "." + sheet[sheetCol.fullName + i]['v']; // データ型（type）
      // チェックボックスのデフォルトはfalseで固定

      value = sheet[sheetCol.type + i]['v'];

      if (value == "自動採番" || value == "Auto Number") {
        value = "AutoNumber";
      } else if (value == "積み上げ集計" || value == "Roll-Up Summary") {
        value = "Summary";
      } else if (value == "外部参照関係" || value == "External Lookup Relationship") {
        value = "ExternalLookup";
      } else if (value == "参照関係" || value == "Lookup Relationship") {
        value = "Lookup";
      } else if (value == "主従関係" || value == "Master-Detail Relationship") {
        value = "MasterDetail";
      } else if (value == "URL") {
        value = "Url";
      } else if (value == "チェックボックス" || value == "Checkbox") {
        value = "Checkbox";
        fields[cnt].defaultValue = false;
      } else if (value == "テキスト" || value == "Text") {
        value = "Text";
      } else if (value == "テキスト(暗号化) " || value == "Text (Encrypted)") {
        value = "EncryptedText";
      } else if (value == "テキストエリア" || value == "Text Area") {
        value = "TextArea";
      } else if (value == "パーセント" || value == "Percent") {
        value = "Percent";
      } else if (value == "メール" || value == "Email") {
        value = "Email";
      } else if (value == "テキストエリア (リッチ)" || value == "Text Area (Rich)") {
        value = "Html";
      } else if (value == "ロングテキストエリア" || value == "Text Area (Long)") {
        value = "LongTextArea";
      } else if (value == "数値" || value == "Number") {
        value = "Number";
      } else if (value == "選択リスト" || value == "Picklist") {
        value = "Picklist";
      } else if (value == "選択リスト (複数選択)" || value == "Picklist (Multi-Select)") {
        value = "MultiselectPicklist";
      } else if (value == "地理位置情報" || value == "Geolocation") {
        value = "Location";
        ...
        value = "Date";
      } else if (value == "日付/時間" || value == "Date/Time") {
        value = "DateTime";
      }
      ...
      */
      ...
      if (sheet[sheetCol.valueSetName + i]) {
        fields[cnt].valueSet = {
          valueSetName: sheet[sheetCol.valueSetName + i]['v']
        };
      ...
      if (sheet[sheetCol.valueSet_valueSetDefinition_value + i]) {
        ...
        for (let j = 0; j < listdata.length; j++) {
          if (j == 0) {
            // ループの初回のみデフォルト設定の判定を行う
            if (sheet[sheetCol.valueSetDefault + i]) {
              valueSetDefault = true;
            } //先頭行デフォルト設定　記述があればtrue
      ...
      */
      ...
      if (sheet[sheetCol.summaryOperation + i]) {
        if (sheet[sheetCol.summaryOperation + i]['v'] == '件数' || sheet[sheetCol.summaryOperation + i]['v'] == 'COUNT') {
          fields[cnt].summaryOperation = 'Count';
        } else if (sheet[sheetCol.summaryOperation + i]['v'] == '合計' || sheet[sheetCol.summaryOperation + i]['v'] == 'SUM') {
          fields[cnt].summaryOperation = 'Sum';
        } else if (sheet[sheetCol.summaryOperation + i]['v'] == '最大' || sheet[sheetCol.summaryOperation + i]['v'] == 'MAX') {
          fields[cnt].summaryOperation = 'Max';
        } else if (sheet[sheetCol.summaryOperation + i]['v'] == '最小' || sheet[sheetCol.summaryOperation + i]['v'] == 'MIN') {
          fields[cnt].summaryOperation = 'Min';
        }
      }
      /*
      件数  COUNT
      ...
      */
      ...
      if (sheet[sheetCol.referenceTo + i]) {
        fields[cnt].referenceTo = sheet[sheetCol.referenceTo + i]['v']; // 参照関係ラベルの自動指定
        ...
        fields[cnt].relationshipName = "Relation_" + fromName + "_to_" + toName; //参照関係のときに削除オプションを設定する

        if (sheet[sheetCol.type + i]['v'] == "参照関係" || sheet[sheetCol.type + i]['v'] == "Lookup Relationship") {
          if (sheet[sheetCol.required + i]) {
            //必須項目の場合「参照関係に含まれる参照レコードは削除できません。」に設定
            fields[cnt].deleteConstraint = "Restrict";
          } else {
            //必須項目ではない場合、「この項目の値をクリアします。 この項目を必須にした場合、このオプションは選択できません。」に設定
...
function getProfilesFromXslx() {
  ...
  if (sheet[sheetCol.profileName1]) {
    profiles.push(sheet[sheetCol.profileName1]['v']);
  }

  if (sheet[sheetCol.profileName2]) {
    profiles.push(sheet[sheetCol.profileName2]['v']);
  }

  if (sheet[sheetCol.profileName3]) {
    profiles.push(sheet[sheetCol.profileName3]['v']);
  }

  if (sheet[sheetCol.profileName4]) {
    profiles.push(sheet[sheetCol.profileName4]['v']);
  }

  if (sheet[sheetCol.profileName5]) {
    profiles.push(sheet[sheetCol.profileName5]['v']);
  }

  if (sheet[sheetCol.profileName6]) {
    profiles.push(sheet[sheetCol.profileName6]['v']);
  }

  if (sheet[sheetCol.profileName7]) {
    profiles.push(sheet[sheetCol.profileName7]['v']);
  }

  if (sheet[sheetCol.profileName8]) {
    profiles.push(sheet[sheetCol.profileName8]['v']);
  }

  if (sheet[sheetCol.profileName9]) {
    profiles.push(sheet[sheetCol.profileName9]['v']);
  }

  if (sheet[sheetCol.profileName10]) {
    profiles.push(sheet[sheetCol.profileName10]['v']);
  }
...
function getPermissionsFromXslx(profiles) {
  ...
  for (let j = 0; j < profiles.length; j++) {
    ...
    permissions.push({
      profilePermisson: []
    }); // プロファイル毎にの中にカスタム項目セキュリティ設定を入れる配列を作る
    // プロファイル毎のカスタム項目セキュリティ設定を入れる配列にjsforce用のオブジェクトを入れていく

    for (let i = 7; i <= excelCol; i++) {
      //主従関係、数式、必須項目は処理から外す
      if (sheet[sheetCol.label + i] != null && sheet[sheetCol.type + i]['v'] != "主従関係" && sheet[sheetCol.type + i]['v'] != "Master-Detail Relationship" && sheet[sheetCol.required + i] == null) {
        permissions[j].profilePermisson.push({}); // 行単位のオブジェクトを追加
        // カスタム項目（field）

        permissions[j].profilePermisson[cnt2].field = sheet['A3']['v'] + "." + sheet[sheetCol.fullName + i]['v']; // editableとreadableを関数から取得　if文の中に書くと長くなるので関数化した

        if (j == 0 && sheet[sheetCol.profile1 + i] != null) {
          set = core.selectPermission(sheet[sheetCol.profile1 + i]['v']);
        }

        if (j == 1 && sheet[sheetCol.profile2 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile2 + i]['v']);
        }

        if (j == 2 && sheet[sheetCol.profile3 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile3 + i]['v']);
        }

        if (j == 3 && sheet[sheetCol.profile4 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile4 + i]['v']);
        }

        if (j == 4 && sheet[sheetCol.profile5 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile5 + i]['v']);
        }

        if (j == 5 && sheet[sheetCol.profile6 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile6 + i]['v']);
        }

        if (j == 6 && sheet[sheetCol.profile7 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile7 + i]['v']);
        }

        if (j == 7 && sheet[sheetCol.profile8 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile8 + i]['v']);
        }

        if (j == 8 && sheet[sheetCol.profile9 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile9 + i]['v']);
        }

        if (j == 9 && sheet[sheetCol.profile10 + i] != null) {
          set = selectPermission(sheet[sheetCol.profile10 + i]['v']);
        } // 編集権限（editable）


        permissions[j].profilePermisson[cnt2].editable = set[0]; // 参照権限（readable）
